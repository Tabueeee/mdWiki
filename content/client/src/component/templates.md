# G:/dev/01_projects/mdWiki/client/src/component/templates.twig
```twig
<template id="nav-menu-template">
    {% include '../component/nav-menu/nav-menu-template.html' %}
</template>
<template id="theme-selector-template">
    {% include '../component/theme-selector/theme-selector-template.html' %}
</template>
<template id="body-message-template">
    {% include '../component/body-message/body-message-template.html' %}
</template>
<template id="search-input-template">
    {% include '../component/search-input/search-input-template.html' %}
</template>
<template id="loading-indicator-template">
    {% include '../component/loading-indicator/loading-indicator-template.html' %}
</template>
<template id="search-results-template">
    {% include '../component/search-results/search-results-template.html' %}
</template>
 ```